...
public class HdhomerunUI implements HdhomerunSignalMeterUiInt,
                                    IndeterminateProgressBarInt,
                                    DeviceListInt,
                                    TunerStatusObserverInt,
                                    ChannelMapObserverInt,
                                    ChannelMapListChangedObserverInt,
                                    ChannelChangedObserverInt,
                                    ProgramObserverInt,
                                    ProgramListObserverInt
{
   ...
   public void buildUIElements()
   {
      ...
      //if channelEditText is not null that probably means we're in a configuration change
      ...
      //Initializing all the spinners are similar to each other, if it already exists that means we are mid
      //reconfiguration so save off the state and set the new one back to that state
   ...
   {
      //There seems to be a bug with progressbar where it doesn't update if you set the same
   ...
   public void stop()
   {
      ...
      mProgramSpinner.setAdapter(new ArrayAdapter<String>(
            mMainActivity, android.R.layout.simple_spinner_item,
            new String[0] ));
      mChannelMapSpinner.setAdapter(new ArrayAdapter<String>(
            mMainActivity, android.R.layout.simple_spinner_item,
            new String[0] ));
      mDeviceSpinner.setAdapter(new ArrayAdapter<String>(
            mMainActivity, android.R.layout.simple_spinner_item,
            new String[0] ));
   }
   ...
   {
      if( (setChannelMapIdx >= 0) &&
          (setChannelMapIdx < mChannelMapSpinner.getCount()) )
      {
   ...
   public void setProgramSpinnerPos(int programPosition)
   {
      if( (programPosition >= 0) &&
          (programPosition < mProgramSpinner.getCount()) )
      {
   ...
   public void setDeviceList( HdhomerunDiscoverDeviceArray aDeviceList )
   {
      ...
      ArrayAdapter<HdhomerunDiscoverDevice> deviceArrayAdapter = new ArrayAdapter<HdhomerunDiscoverDevice>(mMainActivity,
            android.R.layout.simple_spinner_item, aDeviceList.getDiscoverDeviceList());
   ...
   @Override
   public void channelMapChanged( DeviceResponse aResponse, DeviceController aDeviceController,
         String aNewChannelMap )
   {
      @SuppressWarnings("unchecked")
      ArrayAdapter< String > theAdapter = (ArrayAdapter< String >) mChannelMapSpinner.getAdapter();
   ...
   @Override
   public void programChanged( DeviceResponse aResponse,
         DeviceController aDeviceController, ChannelScanProgram aChannelScanProgram )
   {
      
      @SuppressWarnings("unchecked")
      ArrayAdapter< ChannelScanProgram > theAdapter = (ArrayAdapter< ChannelScanProgram >) mProgramSpinner.getAdapter();
   ...
   @Override
   public void channelMapListChanged( DeviceController aDeviceController,
         String[] aChannelMapList )
   {
